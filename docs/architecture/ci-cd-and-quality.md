...
flowchart TD
    ...
    subgraph StrykerRun["Stryker.NET Mutation Testing Engine (9 Modular Configs)"]
        ...
        InboxMut["dotnet-stryker -f stryker-config.inbox.json"]
        OutboxMut["dotnet-stryker -f stryker-config.outbox.json"]
        TestingMut["dotnet-stryker -f stryker-config.testing.json"]
        
        ContractsMut --> CoreMut --> GenMut --> SerMut --> CloudMut --> OTelMut --> InboxMut --> OutboxMut --> TestingMut
    end
...
* **Core Steps:**
  ...
  2. Executes mutation runs for 5 core modules in CI (`core`, `generators`, `serialization`, `cloudevents`, `opentelemetry`). The remaining 4 modules (`contracts`, `inbox`, `outbox`, `testing`) are run locally — see [CONTRIBUTING.md](../../CONTRIBUTING.md) for the full command set.
...
| **Mutation Testing** | Stryker.NET (9 modular configs) | `break: 95` (98.74% actual) | ✅ Enforced |
