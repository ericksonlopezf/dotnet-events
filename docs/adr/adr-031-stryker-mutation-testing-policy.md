...
1. **Modular Stryker Configuration per Package:** Maintain independent `stryker-config.{module}.json` configuration files for each package (`core`, `generators`, `serialization`, `inbox`, `outbox`, `cloudevents`, `opentelemetry`, `testing`).
