...
public static class EventPublisherExtensions
{
        ...
        where TEvent : IEvent
    {
        ...
        return publisher.PublishEnvelopeAsync((IEventEnvelope<TEvent>)envelope, cancellationToken);
    }

    /// <summary>
    /// Publishes an event wrapped in an <see cref="IEventEnvelope{TEvent}"/> to registered subscribers asynchronously.
    /// </summary>
    /// <typeparam name="TEvent">The type of the event contained in the envelope.</typeparam>
    /// <param name="publisher">The event publisher instance.</param>
    /// <param name="envelope">The envelope wrapping the event payload to publish.</param>
    ...
    public static ValueTask PublishAsync<TEvent>(
        this IEventPublisher publisher,
        IEventEnvelope<TEvent> envelope,
        CancellationToken cancellationToken = default)
        where TEvent : IEvent
    {
        ArgumentNullException.ThrowIfNull(publisher);
        ...
        return publisher.PublishEnvelopeAsync(envelope, cancellationToken);
    }
}
