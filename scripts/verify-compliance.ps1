...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.Events.
.DESCRIPTION
    Validates:
...
$allDocFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.md" -ErrorAction SilentlyContinue | Where-Object {
    $_.FullName -notmatch '[\\/](obj|bin|MEGA-AUDIT|tools|\.git|StrykerOutput|TestResults|BenchmarkDotNet\.Artifacts|coverage-results|CoverageReport|temp-coverage|temp-coverage-report)[\\/]'
}
$badDocNames = 0
if ($allDocFiles) {
    foreach ($doc in $allDocFiles) {
        if ($standardExempt -contains $doc.Name) { continue }
        $filename = $doc.Name
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-events\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-events." -ForegroundColor Green }
...
# 8. Zero #pragma warning disable CS0618/CS0619/CS1591 in entire codebase
Write-Host "`n[8/8] Checking for prohibited #pragma warning disable suppressions..." -ForegroundColor Yellow
$allCsFiles = Get-ChildItem -Path (Join-Path $RootDirectory "src"), (Join-Path $RootDirectory "tests"), (Join-Path $RootDirectory "samples"), (Join-Path $RootDirectory "benchmarks") -Recurse -Filter "*.cs" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch "\\(obj|bin)\\" }
$illegalPragmas = 0
foreach ($cs in $allCsFiles) {
    $lines = Get-Content $cs.FullName
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "#pragma\s+warning\s+disable\s+(?:.*?\b)?(CS0618|CS0619|CS1591)\b") {
            Write-Host "  ❌ Prohibited warning suppression in $($cs.FullName):$($i + 1)" -ForegroundColor Red
            $violations++
            $illegalPragmas++
        }
    }
}
if ($illegalPragmas -eq 0) { Write-Host "  ✅ Zero prohibited #pragma warning suppressions found across codebase." -ForegroundColor Green }

# Gate: PackageProjectUrl and ImplicitUsings in Directory.Build.props
Write-Host "`n[Gate: Central Properties] Validating Directory.Build.props settings..." -ForegroundColor Yellow
$propsPath = Join-Path $RootDirectory "Directory.Build.props"
if (Test-Path $propsPath) {
    $propsContent = Get-Content $propsPath -Raw
    if ($propsContent -notmatch "<PackageProjectUrl>https://ericksonlopez\.dev/dotnet-events</PackageProjectUrl>") {
        Write-Host "  ❌ PackageProjectUrl must be https://ericksonlopez.dev/dotnet-events in Directory.Build.props" -ForegroundColor Red
        $violations++
    } else {
        Write-Host "  ✅ PackageProjectUrl is correctly set to https://ericksonlopez.dev/dotnet-events." -ForegroundColor Green
    }
    if ($propsContent -notmatch "<ImplicitUsings>enable</ImplicitUsings>") {
        Write-Host "  ❌ ImplicitUsings must be enable in Directory.Build.props" -ForegroundColor Red
        $violations++
    } else {
        Write-Host "  ✅ ImplicitUsings is correctly enabled." -ForegroundColor Green
    }
}

# Gate: Workflow Canonical Headers
Write-Host "`n[Gate: Workflows] Validating MIT license headers on GitHub Actions workflows..." -ForegroundColor Yellow
$workflowFiles = Get-ChildItem -Path (Join-Path $RootDirectory ".github/workflows") -Filter "*.yml" -ErrorAction SilentlyContinue
$badWorkflows = 0
foreach ($wf in $workflowFiles) {
    $first = Get-Content $wf.FullName -TotalCount 1
    if ($first -notmatch "Copyright .* Erickson Lopez.* MIT License") {
        Write-Host "  ❌ Missing MIT license header in workflow: $($wf.Name)" -ForegroundColor Red
        $violations++
        $badWorkflows++
    }
}
if ($badWorkflows -eq 0) { Write-Host "  ✅ All GitHub Actions workflows include canonical MIT header." -ForegroundColor Green }
