...
public sealed class ArchitectureRulesTests
{
    ...
    private static readonly Assembly InboxAssembly = typeof(EricksonLopez.Events.Inbox.IdempotentEventHandler<>).Assembly;
    private static readonly Assembly OutboxAssembly = typeof(EricksonLopez.Events.Outbox.OutboxEventPublisher).Assembly;
    ...
    [Fact]
    public void Inbox_WithForbiddenDependencies_ShouldNotDependOnInfrastructureOrBrokers()
    {
        var forbiddenDependencies = new[]
        {
            "Microsoft.AspNetCore",
        ...
        var result = Types.InAssembly(InboxAssembly)
            .ShouldNot()
            .HaveDependencyOnAny(forbiddenDependencies)
            .GetResult();

        result.IsSuccessful.Should().BeTrue("Inbox adapter package must not have direct dependencies on message broker SDKs or ORMs.");
    }

    [Fact]
    public void Outbox_WithForbiddenDependencies_ShouldNotDependOnInfrastructureOrBrokers()
    {
        var forbiddenDependencies = new[]
        {
            "Microsoft.AspNetCore",
        ...
        var result = Types.InAssembly(OutboxAssembly)
            .ShouldNot()
            .HaveDependencyOnAny(forbiddenDependencies)
            .GetResult();

        result.IsSuccessful.Should().BeTrue("Outbox adapter package must not have direct dependencies on message broker SDKs or ORMs.");
    }
    ...
    [Fact]
    public void InboxTypes_WhenInAssembly_ShouldResideInEricksonLopezEventsInboxNamespace()
    {
        var result = Types.InAssembly(InboxAssembly)
            .That()
            .ArePublic()
            .Should()
            .ResideInNamespaceMatching(@"^EricksonLopez\.Events\.Inbox.*")
            .GetResult();
    ...
    [Fact]
    public void OutboxTypes_WhenInAssembly_ShouldResideInEricksonLopezEventsOutboxNamespace()
    {
        var result = Types.InAssembly(OutboxAssembly)
            .That()
            .ArePublic()
            .Should()
            .ResideInNamespaceMatching(@"^EricksonLopez\.Events\.Outbox.*")
            .GetResult();
